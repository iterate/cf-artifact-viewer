...
import { getTree, getLog, getBlob, commitChanges, restoreCommit } from "~/functions/git.ts";
...
export const Route = createFileRoute("/$artifact")({
  ...
  loader: async ({ params, deps }) => {
    // Server functions called from loader run server-side
...
function ArtifactView() {
  ...
  useEffect(() => {
    ...
    getBlob({ data: { repo: artifact, path: file, oid: selectedCommit } }).then((content) => {
      ...
      if (isHead) {
        setHead((h) => ({ ...h, [file]: content }));
        setWorking((w) => ({ ...w, [file]: w[file] ?? content }));
      }
    });
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <div className="flex-1 overflow-auto">
          {fileLoading ? (
            ...
            <CodeMirror key={file} value={editorValue} height="100%" theme="dark" extensions={extensions} readOnly={!isHead}
      ...
      <div className="w-[280px] border-l border-[#30363d] overflow-auto text-[13px] shrink-0">
        ...
        {hasLocalChanges && (
          <div className="px-3 py-2 border-b border-[#21262d]">
            ...
            <button className="w-full bg-green-700 hover:bg-green-600 text-white border-none rounded-md py-1.5 px-3 cursor-pointer text-[13px] disabled:opacity-50" onClick={handleCommit} disabled={!commitMsg.trim()}>
              Commit &amp; push {dirty.size} file{dirty.size !== 1 ? "s" : ""}
            </button>
          </div>
        )}
        {hasLocalChanges && (
          <div onClick={() => nav({ file })} className="px-3 py-2 border-b border-[#21262d] bg-[#161b22] border-l-[3px] border-l-orange-400 cursor-pointer">
        ...
        {commits.map((c: { oid: string; message: string; author: string; timestamp: number }, i: number) => {
          ...
          return (
            <div key={c.oid} className={`px-3 py-2 border-b border-[#21262d] ${isActive ? "bg-[#161b22]" : ""}`}>
              ...
              {!isLatest && selectedCommit === c.oid && (
                <button className="mt-1 bg-transparent text-blue-400 border border-[#30363d] rounded px-2 py-0.5 cursor-pointer text-xs hover:bg-[#161b22]" onClick={() => handleRestore(c.oid)}>Restore to this commit</button>
              )}
            </div>
          );
        })}
...
function FileTree({ nodes, depth, selected, dirty, expanded, onSelect, onToggle }: {
  ...
  expanded: Set<string>; onSelect: (path: string) => void; onToggle: (path: string) => void;
}) {
  return (
    <>
      {nodes.map((node) => {
        ...
        return (
          <div key={node.path}>
            ...
            {isFolder && isOpen && <FileTree nodes={node.children} depth={depth + 1} selected={selected} dirty={dirty} expanded={expanded} onSelect={onSelect} onToggle={onToggle} />}
          </div>
        );
      })}
    </>
