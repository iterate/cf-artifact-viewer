...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    // Optional HTTP basic auth — set BASIC_AUTH_PASSWORD env var to enable
    if (env.BASIC_AUTH_PASSWORD && p.startsWith("/api/")) {
      const auth = req.headers.get("Authorization");
      if (!auth?.startsWith("Basic ")) return new Response("Unauthorized", { status: 401, headers: { "WWW-Authenticate": 'Basic realm="Artifacts"' } });
      const [, password] = atob(auth.slice(6)).split(":");
      if (password !== env.BASIC_AUTH_PASSWORD) return new Response("Forbidden", { status: 403 });
    }
